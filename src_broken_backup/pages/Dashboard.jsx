...
function pin(color) {
  return L.divIcon({
    className: "",
    html: `<span style="display:block;width:15px;height:15px;border-radius:50% 50% 50% 0;transform:rotate(-45deg);background:${color};border:2px solid #fff;box-shadow:0 1px 4px rgba(0,0,0,.4)"></span>`,
    iconSize: [15, 15],
    iconAnchor: [7, 15],
  });
}
...
export default function Dashboard() {
  ...
  const mapCenter = useMemo(() => {
    if (located[0]) return [located[0].last_latitude, located[0].last_longitude];
    const firstArea = workAreas.find((w) => w.boundary_points?.length);
    if (firstArea) return [firstArea.boundary_points[0].latitude, firstArea.boundary_points[0].longitude];
    ...
    <div className="page" style={{ maxWidth: "none" }}>
      ...
      <div className="dash-layout">
        <div className="dash-map-card">
          <MapContainer center={mapCenter} zoom={13} style={{ height: "100%", width: "100%" }} className="map-shell">
            <TileLayer
              attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
              url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
            />
            {workAreas.map((wa) => {
              const positions = (wa.boundary_points || []).map((p) => [p.latitude, p.longitude]);
              if (positions.length < 3) return null;
              return (
                <Polygon
                  key={wa.id}
                  positions={positions}
                  pathOptions={{ color: wa.color || "#0F6E5C", weight: 1.5, fillOpacity: 0.1, dashArray: "5 4" }}
                >
                  <Popup>
